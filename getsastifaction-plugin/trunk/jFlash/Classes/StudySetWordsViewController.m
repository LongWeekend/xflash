...
@synthesize tag, cards, queue, statusMsgBox, studySetWordsTable;
...
- (void) viewWillAppear: (BOOL)animated
{
  ...
  [[self studySetWordsTable] setBackgroundColor: [UIColor clearColor]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  {
    cell = [self reuseCellForIdentifier:CellIdentifier onTable:tableView usingStyle:UITableViewCellStyleSubtitle];
    cell.selectionStyle = 0;
    cell.detailTextLabel.text = [[cards objectAtIndex:indexPath.row] headword];
    cell.textLabel.text = [[cards objectAtIndex:indexPath.row] meaningWithoutMarkup];
  }
  else
  {
    cell = [self reuseCellForIdentifier:HeaderIdentifier onTable:tableView usingStyle:UITableViewCellStyleSubtitle];
...
/*
 TODO
 [tag release]; /// for some reason we cannot release this without breaking the 
*/
