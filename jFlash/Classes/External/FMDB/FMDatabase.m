...
- (BOOL) open {
  ...
  if (sqlite3_exec(db, "PRAGMA CACHE_SIZE=6000;", NULL, NULL, NULL) != SQLITE_OK) {
    NSAssert1(0, @"Error: failed to set cache size with message '%s'.", sqlite3_errmsg(db));
  }
  if (sqlite3_exec(db, "PRAGMA synchronous=1;", NULL, NULL, NULL) != SQLITE_OK) {
    NSAssert1(0, @"Error: failed to set synchronuous with message '%s'.", sqlite3_errmsg(db));
  }
	return YES;
}
