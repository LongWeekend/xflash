...
class BookListParserTest < Test::Unit::TestCase
  ...
  def test_parse_beg_chinese
    expected_entries =[]
    results_data = BookListParser.new(File.dirname(__FILE__) + "/../../../../data/cedict/test_data/beg_chinese.txt").run
    assert_equal(3,results_data.count)
  end

  def test_parse_colloquial_chinese
    expected_entries =[]
    results_data = BookListParser.new(File.dirname(__FILE__) + "/../../../../data/cedict/test_data/coll_chinese.txt").run
    assert_equal(3,results_data.count)
  end

  def test_parse_integrated_chinese
    expected_entries =[]
    results_data = BookListParser.new(File.dirname(__FILE__) + "/../../../../data/cedict/test_data/integrated_chinese.txt").run
    assert_equal(10,results_data.count)
  end

  def test_parse_schaums
    expected_entries =[]
    results_data = BookListParser.new(File.dirname(__FILE__) + "/../../../../data/cedict/test_data/schaums.txt").run
