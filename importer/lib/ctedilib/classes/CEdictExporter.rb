...
class CEdictExporter
  ...
  def sqlite_create_rikai
    ...
    \\
    DROP INDEX IF EXISTS cards_ptag;\\
    CREATE INDEX cards_ptag ON dict (priority_word ASC);\\
    \\
    DROP INDEX IF EXISTS cards_headwords;\\
    CREATE INDEX cards_headwords ON dict (headword_trad, headword_simp ASC);\\
    \\
    END TRANSACTION;\\
    BEGIN TRANSACTION;\\
    \\"
  ...
  def sqlite_create_fts
    ...
    CREATE VIRTUAL TABLE cards_search_content using FTS3(card_id, headword, reading, content, ptag);\\
    \\
    DROP TABLE IF EXISTS version;\\
    CREATE TABLE version (plugin_key TEXT PRIMARY KEY NOT NULL, version TEXT, plugin_name TEXT);\\
