...
+ (int) createTag: (NSString*) tagName withOwner: (NSInteger) ownerId
{
  ...
  [sql release];
  int lastTagId = (int)appSettings.dao.lastInsertRowId;
  if (appSettings.dao.hadError == NO)
  {
    // Link it
...
{
  ApplicationSettings *appSettings = [ApplicationSettings sharedApplicationSettings];
	FMDatabase *dao = [appSettings dao];
...
+ (void) subscribe: (NSInteger) cardId tagId: (NSInteger) tagId
{
  ...
  NSString *sql2 = [[NSString alloc] initWithFormat:@"UPDATE tags SET count=(count+1) WHERE tag_id = %d",tagId];
  [appSettings.dao beginTransaction];
  [appSettings.dao executeUpdate:sql];
  [appSettings.dao executeUpdate:sql2];
  [appSettings.dao commit];
	[sql release];
	[sql2 release];
  if ([appSettings.dao hadError])
  {
    NSLog(@"Err %d: %@", [appSettings.dao lastErrorCode], [appSettings.dao lastErrorMessage]);
  }
}
...
+ (BOOL) deleteTag:(NSInteger) tagId
{
  ...
  FMResultSet *rs = [appSettings.dao executeQuery:sql4];
  [sql4 release];
  int groupId = 0;
	while ([rs next])
  {
  ...
  [appSettings.dao executeUpdate:sql];
  [appSettings.dao executeUpdate:sql2];
  [appSettings.dao executeUpdate:sql3];
  [appSettings.dao commit];
	[sql release];
	[sql2 release];
	[sql3 release];
